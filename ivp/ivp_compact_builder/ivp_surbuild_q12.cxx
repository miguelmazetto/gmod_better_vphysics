...
void IVP_SurfaceBuilder_Q12::convert_node(size_t node)
{
...
void IVP_SurfaceBuilder_Q12::nodes_to_planes()
{
	...
	dplane_t *bsp_plane = &dplanes[dnodes[(size_t)nodes.element_at(i)].planenum];
	if ( i == this->nodes.len()-1 ) {
	    if ( dnodes[(size_t)this->nodes.element_at(i)].children[0] == -1 ) {
	    ...
	    if ( dnodes[(size_t)this->nodes.element_at(i)].children[0] == (size_t)this->nodes.element_at(i+1) ) {
		create_and_insert_plane( bsp_plane->normal[0],  bsp_plane->normal[1],  bsp_plane->normal[2], bsp_plane->dist);
	    }
	    else {
		create_and_insert_plane(-bsp_plane->normal[0], -bsp_plane->normal[1], -bsp_plane->normal[2], -bsp_plane->dist);
