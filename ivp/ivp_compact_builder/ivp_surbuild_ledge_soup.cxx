...
void IVP_SurfaceBuilder_Ledge_Soup::cluster_spheres_bottomup(IVP_DOUBLE threshold_increase) {
	...
	while ( this->interval_minhash->counter > 0 ) {
		...
		key.key = (size_t)cluster_min_hash.find_min_elem();
		if ( key.key != (size_t)NULL ) { // verify that there were at least 2 spheres in vector and a new minimal sphere could be generated!
...
void IVP_SurfaceBuilder_Ledge_Soup::generate_interval_minhash(float fixed_max_radius) {
	...
	new_key.key = (size_t)new_entry_interval_start;
	...
	new_key.key = (size_t)new_entry_interval_end;
	this->interval_minhash->add((void *)new_key.key, sphere->center.k[this->longest_axis]+radius);
...
void IVP_SurfaceBuilder_Ledge_Soup::ledgetree_array_debug_output() {
	...
	node = &nodes[i];
	printf("Node %d (address: 0x%zx / %zu)\n", i, (size_t)node, (size_t)node);
	//node->center.print("     center ");
	printf("        radius %.6f)\n", node->radius);
	printf("         left branch offset: %zu (address: 0x%zx / %zu)\n", sizeof(*node), (size_t)(node+1), (size_t)(node+1));
	printf("        right branch offset: %d (address: 0x%zx / %zu)\n", node->offset_right_node, (size_t)node+node->offset_right_node, (size_t)node+node->offset_right_node);
	printf("\n");
    }
    // *** debugging END ********************************************************
    return;
