...
realT qh_detsimplex(pointT *apex, setT *points, int dim, boolT *nearzero) {
  ...
  trace2((qh ferr, "qh_detsimplex: det=%2.2g for point p%zu, dim %d, nearzero? %d\n",
...
realT qh_facetarea_simplex (int dim, coordT *apex, setT *vertices, 
  ...
  if (toporient)
    area= -area;
  area *= qh AREAfactor;
  trace4((qh ferr, "qh_facetarea_simplex: area=%2.2g for point p%zu, toporient %d, nearzero? %d\n",
...
facetT *qh_findgooddist (pointT *point, facetT *facetA, realT *distp, 
  ...
  *facetlist= facetA;
  ...
  if (bestfacet) {
    *distp= bestdist;
    trace2((qh ferr, "qh_findgooddist: p%zu is %2.2g above good facet f%d\n",
      qh_pointid(point), bestdist, bestfacet->id));
    return bestfacet;
  }
  trace4((qh ferr, "qh_findgooddist: no good facet for p%zu above f%d\n", 
  ...
  for(k= sizinit; k < dim+1; k++) {
    ...
    if (!maxpoint || maxnearzero) {
      ...
      if (!maxpoint) {
        ...
        trace0((qh ferr, "qh_maxsimplex: searching all points for %d-th initial vertex, better than p%zu det %2.2g\n",
    ...
    trace1((qh ferr, "qh_maxsimplex: selected point p%zu for %d`th initial vertex, det=%2.2g\n",
...
void qh_printpoints (FILE *fp, const char *string, setT *points) {
  ...
  if (string) {
    fprintf (fp, "%s", string);
    FOREACHpoint_(points) 
      fprintf (fp, " p%zu", qh_pointid(point));
    fprintf (fp, "\n");
  }else {
    FOREACHpoint_(points) 
