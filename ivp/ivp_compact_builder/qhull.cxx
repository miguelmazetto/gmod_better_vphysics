...
boolT qh_addpoint (pointT *furthest, facetT *facet, boolT checkdist) {
  ...
  if (qh STOPpoint > 0 && qh furthest_id == qh STOPpoint-1) 
    return False; 
  qh_resetlists (True /*qh visible_list newvertex_list newfacet_list */);
  trace2((qh ferr, "qh_addpoint: added p%zu new facets %d new balance %2.2g point balance %2.2g\n",
...
void qh_buildhull(void) {
  ...
  FORALLvertices {
	...
	(qh STOPcone>0 && id == qh STOPcone-1)) {
      trace1((qh ferr,"qh_buildhull: stop point or cone P%zu in initial hull\n", id));
      return;
    }
  }
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 ...
 outside points.  Next is point p%zu (v%d), %2.2g above f%d.\n",
#ifndef GEKKO
         tp->tm_hour, tp->tm_min, tp->tm_sec,
#else
		 0,0,0,
    ...
    cpu /= qh_SECticks;
    qh_distplane (furthest, facet, &dist);
    ivp_message( "qh_addpoint: add p%zu (v%d) to hull of %d facets (%2.2g above f%d) and %d outside at %4.4g CPU secs.  Previous was p%zu.\n",
...
void qh_findhorizon(pointT *point, facetT *facet, int *goodvisible, int *goodhorizon) {
  ...
  trace1((qh ferr,"qh_findhorizon: find horizon for point p%zu facet f%d\n",qh_pointid(point),facet->id));
	  ...
	  if (qh MERGING) {
	    if (dist > 0) {
	      ...
	      minimize_(qh min_vertex, dist);  /* due to merge later */
      	  ...
      	  trace2((qh ferr, "qh_findhorizon: point p%zu is coplanar to horizon f%d, dist=%2.7g < qh MINvisible (%2.7g)\n",
...
Point p%zu was above all facets.\n", qh_pointid(point));
...
void qh_partitionall(setT *vertices, pointT *points, int numpoints){
  ...
  int size, point_i, point_n, point_end, remaining, i;
  size_t id;
  facetT *facet;
...
void qh_partitioncoplanar (pointT *point, facetT *facet, realT *dist) {
  ...
  if (istrace) {
    ivp_message( "qh_partitioncoplanar: ====== p%zu increases max_outside to %2.2g of f%d last p%d\n",
		   qh_pointid(point), bestdist, bestfacet->id, qh furthest_id);
    qh_errprint ("DISTANT", bestfacet, NULL, NULL, NULL);
  }
  trace4((qh ferr, "qh_partitioncoplanar: point p%zu is coplanar with facet f%d (or inside) dist %2.2g\n",
...
void qh_partitionpoint (pointT *point, facetT *facet) {
  ...
  if (isoutside) {
    ...
    qh num_outside++;
    trace4((qh ferr, "qh_partitionpoint: point p%zu is outside facet f%d\n",
    ...
    zinc_(Zpartinside);
    trace4((qh ferr, "qh_partitionpoint: point p%zu is inside all facets, closest to f%d dist %2.2g\n",
	  qh_pointid(point), bestfacet->id, bestdist));
    if (qh KEEPinside)	  
      qh_partitioncoplanar (point, bestfacet, &bestdist);
  }
...
void qh_printsummary(FILE *fp) {
  ...
  float cpu;
  int size, nummerged, numvertices, numcoplanars= 0;
  size_t id;
  facetT *facet;
  const char *s;
	if (!fp ) return;
