...
void qh_check_maxout (void) {
  ...
  && (qh PRINTsummary || qh KEEPinside || qh KEEPcoplanar 
	...
	|| neighbor == qh tracefacet || vertex == qh tracevertex)
	  ivp_message( "qh_check_maxout: p%zu (v%d) is %.2g from f%d\n",
  ...
  FOREACHfacet_i_(facets) {     /* for each point with facet assignment */
	...
	ivp_message( "qh_check_maxout: p%zu is %.2g above f%d\n",
...
void qh_check_point (pointT *point, facetT *facet, realT *maxoutside, realT *maxdist, facetT **errfacet1, facetT **errfacet2) {
  ...
  if (dist > *maxoutside) {
    *errfacet2= *errfacet1;
    *errfacet1= facet;
    ivp_message( "qhull precision error: point p%zu is outside facet f%d, distance= %6.8g maxoutside= %6.8g\n", 
	  ...
	  if (fault == qh_DATAfault) {
	    ...
	    qh_errexit(qh_ERRsingular, NULL, NULL);
          ...
          if (dist > qh DISTround) {
    	    ...
    	    if (qh ferr){
		ivp_message( "qhull precision error: f%d is concave to f%d, since p%zu (v%d) is %6.4g above\n",
	      ...
	      if (qh ferr){
		  ivp_message( "qhull precision error: f%d is clearly not convex to f%d, since p%zu (v%d) is %6.4g above\n",
		    facet->id, neighbor->id, qh_pointid(vertex->point), vertex->id, dist);
	      }
              errfacet1= facet;
              errfacet2= neighbor;
            ...
            zzinc_(Zcoplanarridges);
            qh_precision ("coplanar ridge");
            trace0((qh ferr, "qhull precision error: f%d may be coplanar to f%d, since p%zu (v%d) is within %6.4g during p%d\n",
  ...
  if (qh GOODvertex <0 || (qh GOODvertex > 0 && qh MERGING)) {
    FORALLfacet_(facetlist) {
      if (facet->good && ((qh GOODvertex > 0) ^ !!qh_isvertex (qh GOODvertexp, facet->vertices))) {
        if (!--numgood) {
          ivp_message( "qhull warning: good vertex p%zu does not match last good facet f%d.  Ignored.\n",
...
void qh_furthestout (facetT *facet) {
  ...
  trace3((qh ferr, "qh_furthestout: p%zu is furthest outside point of f%d\n",
...
vertexT *qh_makenewfacets (pointT *point /*visible_list*/) {
  ...
  trace1((qh ferr, "qh_makenewfacets: created %d new facets from point p%zu to horizon\n",
...
may have the same identifier.  Vertices not sorted correctly.\n", 0xFFFFFF);
  ...
  vertex->point= point;
  trace4((qh ferr, "qh_newvertex: vertex p%zu (v%d) created\n", qh_pointid(vertex->point), 
	  vertex->id));
  return (vertex);
} /* newvertex */
