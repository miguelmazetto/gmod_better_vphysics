...
pointT *qh_detvnorm (vertexT *vertex, vertexT *vertexA, setT *centers, realT *offsetp) {
  facetT *facet, **facetp;
  int  i, k, point_i, point_n;
  size_t pointid, pointidA;
  setT *simplex= NULL;
  ...
  if (qh VERIFYoutput || qh PRINTstatistics) {
    ...
    if (!unbounded) {
      ...
      trace4((qh ferr, "qh_detvnorm: points %zu %zu midpoint dist %2.2g\n",
	...
	angle= angle - 1.0;
      if (angle < 0.0)
	angle -= angle;
      trace4((qh ferr, "qh_detvnorm: points %zu %zu angle %2.2g nearzero %d\n",
    ...
    if (simplex != points) {
	  ...
	  zzinc_(Zridge);
          ...
          wwadd_(Wridge, dist);
          trace4((qh ferr, "qh_detvnorm: points %zu %zu Voronoi vertex %d dist %2.2g\n",
...
setT *qh_detvridge3 (vertexT *atvertex, vertexT *vertex) {
  ...
  if (qh CHECKfrequently) {
    FOREACHneighbor_(vertex) {
      if (!neighbor->seen2) {
	fprintf (stderr, "qh_detvridge3: neigbors of vertex p%zu are not connected at facet %d\n",
    ...
    if (neighbor->seen) {
	  ...
	  vertex->visitid= qh vertex_visit;
          ...
          if (count >= qh hull_dim - 1) {  /* e.g., 3 for 3-d Voronoi */
            ...
            totridges++;
            trace4((qh ferr, "qh_eachvoronoi: Voronoi ridge of %d vertices between sites %zu and %zu\n",
...
void qh_printafacet(FILE *fp, int format, facetT *facet, boolT printall) {
  ...
  case qh_PRINTcoplanars:
    ...
    FOREACHpoint_(facet->coplanarset)
      fprintf (fp, " %zu", qh_pointid (point));
    fprintf (fp, "\n");
    break;
  ...
  case qh_PRINTvertices:
    ...
    FOREACHvertex_(facet->vertices)
      fprintf (fp, " %zu", qh_pointid (vertex->point));
    fprintf (fp, "\n");
    break;
  }
} /* printafacet */
...
void qh_printfacetheader(FILE *fp, facetT *facet) {
  ...
  if (!SETempty_(facet->outsideset)) {
    ...
    if (qh_setsize (facet->outsideset) < 6) {
      fprintf(fp, "    - outside set (furthest p%zu):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->outsideset)
  ...
  if (!SETempty_(facet->coplanarset)) {
    ...
    if (qh_setsize (facet->coplanarset) < 6) {
      fprintf(fp, "    - coplanar set (furthest p%zu):\n", qh_pointid(furthest));
      FOREACHpoint_(facet->coplanarset)
	qh_printpoint(fp, "     ", point);
...
to the center point are as follows:\n\n", qh DISTround);
  ...
  FORALLfacets {
    fprintf (fp, "facet");
    FOREACHvertex_(facet->vertices)
      fprintf (fp, " p%zu", qh_pointid(vertex->point));
    zinc_(Zdistio);
...
void qh_printhyperplaneintersection(FILE *fp, facetT *facet1, facetT *facet2,
  ...
  FOREACHvertex_(vertices) {
    ...
    if (nearzero1+nearzero2)
      fprintf (fp, "p%zu (coplanar facets)\n", qh_pointid (vertex->point));
    else
      fprintf (fp, "projected p%zu\n", qh_pointid (vertex->point));
  }
  if (qh hull_dim == 3)
    fprintf(fp, "%8.4g %8.4g %8.4g 1.0\n", color[0], color[1], color[2]); 
  else if (qh hull_dim == 4 && qh DROPdim >= 0)  
...
/*-<a                             href="qh-c.htm#io"
  ...
  returns:
    if string is defined
      prints 'string p%zu' (skips p%zu if id=-1)
...
void qh_printvertex(FILE *fp, vertexT *vertex) {
  ...
  fprintf(fp, "- p%zu (v%d):", qh_pointid(vertex->point), vertex->id);
  point= vertex->point;
  if (point) {
    for(k= qh hull_dim; k--; ) {
      r= *point++;
...
void qh_printvridge (FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT /*unbounded*/) {
  ...
  fprintf (fp, "%d %zu %zu", qh_setsize (centers)+2, 
