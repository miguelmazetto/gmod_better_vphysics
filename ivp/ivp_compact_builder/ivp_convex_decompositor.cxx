...
void IVP_Concave_Polyhedron_Face::add_offset(size_t offset_in) {
...
class IVP_Convex_Subpart_Work {
public:
    IVP_U_Vector<size_t> offsets; // @@@SF: replace with IVP_VHash someday!!!

    void add_offset(size_t offset_in);
};


void IVP_Convex_Subpart_Work::add_offset(size_t offset_in) {

    int i;
    for (i=0; i<this->offsets.len(); i++) {
	if ( this->offsets.element_at(i) == (size_t*)offset_in ) {
	    return;
	}
    }
    this->offsets.add((size_t *)offset_in);
    return;
}
...
fclose(fp);
	...
	for (i=0; i<npolh_out; i++) {
		...
		size_t offset = (size_t)subpart_work->offsets.element_at(j)-1;
