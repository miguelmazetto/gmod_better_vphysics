...
class IVP_MM_CMP {
public:
    static inline size_t calc_hash_index( IVP_MM_CMP_Key * o){
	size_t x = (size_t)o->ledge[0] ^ ( size_t(o->ledge[1])* 75 );
	return x + 1023 * (x>>8);
    }

    // calc hash index of c, con is a reference
    static inline size_t calc_hash_index( IVP_Collision *c, IVP_MM_CMP_Key * /*ref_key*/){
	const IVP_Compact_Ledge *ledge[2];
	c->get_ledges(ledge);
	size_t x = (size_t)ledge[0] ^ ( size_t(ledge[1])* 75 );
	return x + 1023 * (x>>8);
    }
...
class IVP_OO_CMP {
public:
    static inline size_t calc_hash_index( IVP_Real_Object * o){
	size_t x = (size_t)o;
	return x + 1023 * (x>>8);
    }

    // calc hash index of c, con is a reference
    static inline size_t calc_hash_index( IVP_Collision *c, IVP_Real_Object * con){
	IVP_Real_Object *objects[2];
	c->get_objects(objects);
	size_t x = size_t(objects[0]) ^ size_t(objects[1]) ^ size_t(con);  // take other object (trick to avoid if)
...
void IVP_Mindist::update_exact_mindist_events(IVP_BOOL allow_hull_conversion, IVP_MINDIST_EVENT_HINT event_hint)
{
	IVP_IF(1) 
	{
		...
		{
			fprintf(dm->out_deb_file,"doing_mindist_events %zx at %f\n",0x0000ffff&(size_t)this,get_environment()->get_current_time().get_time());
		}
	}
